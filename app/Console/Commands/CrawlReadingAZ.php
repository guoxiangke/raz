...
//        Storage::makeDirectory($path); //  无字书，不要了！
    ...
    public function getPdf($pdfString){
        ...
        if(preg_match($pattern, $pdfString,$matchs)){
            $pdfUri = $matchs[1];
            $pdfUri = str_replace("_clr.pdf'}",'', $pdfUri);
            $pdfUri = str_replace("_clr_ds.pdf'}",'', $pdfUri);
            return $pdfUri;
        }
        return '';
    }
...
//        }
        ...
        $jsonPath = "ReadingAZ/raz-0921-01.json";
...
//        $bookIdStrings = '29;1309';
        ...
        foreach ($bookIds as $id){
            ...
            $pdfString = $dom->find('a.pdf',1)->getAttribute('require-teacher-login') ; //Book Resources
            $pdf = $this->getPdf($pdfString);
            $pdfs = [];
            ...
            try {
                ...
                if(!Storage::exists($filePath)){
                    try {
                        ...
                        $link = "https://mi.content.readinga-z.com/pdfsite/levels/{$level}_ds/{$pdf}_clr_ds.pdf";
                        try {
                            Storage::put($filePath, file_get_contents($link));
                            Log::info("Double-Sided Book: {$id} Downloaded");
                        }catch (\Exception $e){
...
//                    try {
//                        Storage::put($filePath, file_get_contents($link));
//                        Log::info("All Worksheets: {$id} Downloaded");
//                    }catch (\Exception $e){
...
//                }
            ...
            $data = compact('id','level','title','subtle','description','count','from','meta', 'pdf', 'pdfs');
//            dd($data);
            $json[$id] = $data;
