...
//            Log::info("{$bookId}: {$pageCount} pages downloaded");
            ...
            $pdfString = $dom->find('a.pdf',0)->getAttribute('require-teacher-login') ; //Book Resources
            $pattern = '/([^\/]+$)/'; // 匹配从最后一个/开始的到结尾的 内容
            if(preg_match($pattern, $pdfString,$matchs)){
                $pdfUri = $matchs[1];
                $pdf = str_replace("_clr.pdf'}",'', $pdfUri) ;
            }
