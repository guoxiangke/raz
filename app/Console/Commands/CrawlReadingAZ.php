...
class CrawlReadingAZ extends Command
{
    ...
    public function __construct()
    {
        ...
        $cookieStr = 'BIGipServerlaz_prod_readinga-z=1644224266.6699.0000; lb-SortOption=titleup; book-view=grid; guided-reading-sort-option=titleup; _ga=GA1.2.1391875090.1599700373; _gid=GA1.2.179468730.1599700373; cookietest=success; razKidsDoLogin=yes; teacherUsername=daxiangyingyu; grade=K; bid=b08f68d464f8697c835f485c6c5d0e5e02c33226; lb-filter=%7B%22level-B%22%3A%22%22%7D; PHPSESSID=lnb8a6i36b2pnlvm4oo2vclenf; _uetsid=0022c14f2e0be2641b3b4fb6a98ef8a2; _uetvid=5eb3657c56b14abd8a46765160177ef5; _gat_lazUniversal=1; _gat_lazGtmProduct=1';
    ...
    public function handle()
    {
        ...
        //  $(this).find('.title').next().text()) //Fiction
        // })
...
//            if(isset($json[$bookId])) {
//                Log::warning("Exists: {$bookId}");
//                continue;
//            }
            ...
            $pageCount = $dom->find('#miniBookSlider img')->count();

            $level = strtolower(trim($dom->find('#levelBarNew a.active',0)->innerText()));
            ...
            Log::info("{$bookId}: {$pageCount} page need download");
...
//                // $level = $pdfArray[count($pdfArray)-1];
//
//                // Single-Sided Book
...
//                        $url = $dom->find('a.pdf',2)->getAttribute('href') ; //Book Resources
//                        if(url){
//                            Log::info("Double-Sided Book a.pdf: $url");
//                            $response = Http::withCookies($this->cookie,'www.readinga-z.com')->sink(storage_path("app/$filePath"))->get($url);
//                            if($response->status() !== 200 ){
//                                Log::error("No Double-Sided Book: {$bookId}  wget -O $filePath $pdf");
//                            }
//                        }
//                    }
//                }
...
//                        Storage::put($filePath, file_get_contents($pdf));
//                    }catch (\Exception $e){
//                        Log::warning("No Pocketbook: {$bookId}  wget -O $filePath $pdf");
//                    }
//                }
//
...
//                        Storage::put($filePath, file_get_contents($pdf));
//                    }catch (\Exception $e){
//                        Log::warning("No Read and Color Books: {$bookId} wget -O $filePath $pdf");
//                    }
//
//                }
//                // Writing Resources
...
//                            Storage::put($filePath, file_get_contents($pdf));
//                        }catch (\Exception $e){
//                            Log::warning("No WritingResources: {$bookId}  wget -O $filePath $pdf");
//                        }
//                    }
//
//                }
...
//                                Storage::put($filePath, file_get_contents($pdf));
//                            }catch (\Exception $e){
//                                Log::warning("No LessonResources3: {$bookId} ");
//                            }
//                        }
//                    }
//                }
...
//                                Storage::put($filePath, file_get_contents($pdf));
//                            }catch (\Exception $e){
//                                Log::warning("NO Worksheets3: {$bookId} {$e->getMessage()}");
//
//                            }
//
//                        }
...
//                }
                ...
                Log::warning("Downloaded: {$bookId}");
            }
        }

        }catch (\Exception $e){
            Log::warning("Big Exception: {$e->getMessage()}");
        }
        return 0;
    }
}
